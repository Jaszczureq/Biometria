...
//    private BufferedImage imageCopy;
    ...
    private final JLabel positionValues = new JLabel();
    private final JLabel rgbValues = new JLabel();
...
//    private int[] histImgAvg = new int[n];
    ...
    private pl.edu.pb.wi.funtional.Histogram histogramObj;
    ...
    public Viewer() {
        ...
        jToolBar.add(positionValues);
        jToolBar.add(rgbValues);
        jToolBar.add(zoomValue);
        initActionListeners();
    }
    ...
    private void initActionListeners() {
        ...
        loadImage.addActionListener((ActionEvent e) -> {
            imgObj=new ImageClass(this);
            imgObj.loadImage(null);
            this.imageLabel.setIcon(new ImageIcon(imgObj.getDeepCopyImg()));
            histogramObj=new pl.edu.pb.wi.funtional.Histogram(imgObj);
        });
...
//            }
        ...
        calculateHistograms.addActionListener((ActionEvent e) -> {
            histogramObj.calculateHistograms();
//            if (imgObj.isImageLoaded()) {
//                makeHist(imgObj.getDeepCopyImg());
...
//        int[] arr = new int[(int) Math.pow(2 * r + 1, 2)];
        ...
        for (int k = i - r; k <= i + r; k++) {
            for (int l = j - r; l <= j + r; l++) {
                try {
                    arr.add(imgObj.getOriginalImg().getRGB(k, l));
...
//    }
    ...
    private double standardDiviation(int i, int j, int r, double avg) {
        ...
        for (int k = i - r; k <= i + r; k++) {
            for (int l = j - r; l <= j + r; l++) {
                try {
                    Color c = new Color(imgObj.getOriginalImg().getRGB(k, l));
...
//                }
                ...
                if (dragging) {
                    ...
                    try {
                        BufferedImage temp = imgObj.getOriginalImg().getSubimage(offsetX, offsetY, imageLabel.getWidth(), imageLabel.getHeight());
...
//                jPanel.add(imageLabel, BorderLayout.CENTER);
        ...
        public void mouseMoved(MouseEvent e) {
            try {
                ...
                positionValues.setText(location);
                if (imgObj.isImageLoaded()) {
                    int imagem = imgObj.getOriginalImg().getRGB(e.getX(), e.getY());
                    Color color = new Color(imagem, true);
                    String location2 = String.format("[%d, %d, %d]", color.getRed(), color.getGreen(), color.getBlue());
                    rgbValues.setText(location2);
                    zoomValue.setText("C: " + varC);
